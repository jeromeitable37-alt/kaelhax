...
export default function Home() {
  ...
  const [cloudRole, setCloudRole] =
    useState('member');
  const [cloudPhotoURL, setCloudPhotoURL] =
    useState('');
  ...
  async function loginGoogle() {
    ...
    try {
      ...
      if (credential?.user && db) {
        ...
        if (cloudProfile?.role === 'admin' && cloudProfile?.disabled !== true) {
          flash('Admin account detected. Opening Admin Console…');
  ...
  async function saveUserProfile() {
    ...
    const nextName = profileName.trim() || 'NEXORIUM USER';
    setProfileBusy(true);
    ...
    try {
      if (firebaseConfigured && auth?.currentUser) {
        ...
        const nextSession = { ...currentUser, name: nextName };
  ...
  async function uploadUserProfilePicture(file) {
    ...
    try {
      if (firebaseConfigured && storage && auth?.currentUser) {
        const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '-');
        const uid = auth.currentUser.uid;
        const objectRef = ref(
          storage,
          `profiles/${uid}/${Date.now()}-${crypto.randomUUID()}-${safeName}`
        );
        ...
        throw new Error('Firebase Storage is required for profile photo uploads.');
      }
  ...
  return (
    <div className="site-shell">
      ...
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          {currentUser ? (
            ...
            >
              <span className="account-avatar account-avatar-image">
                {(currentUser.photoURL || cloudPhotoURL) ? (
                  <img src={currentUser.photoURL || cloudPhotoURL} alt="Profile" />
                ) : (
      ...
      >
        ...
        <div className="guest-card">
          ...
          {currentUser ? (
            <button
              type="button"
              className="avatar profile-avatar profile-avatar-button"
              onClick={openUserProfile}
        ...
        {currentUser && (
          <>
            ...
            >
              <Icon>◉</Icon>
              <span>My Orders</span>
              <em>{myOrders.filter((order) => order.status === 'pending').length || ''}</em>
            </button>
      ...
      {showProfile && currentUser && (
        <div
          className="modal-bg"
          onClick={() => !profileBusy && setShowProfile(false)}
        >
          <div
            className="modal profile-user-modal"
          ...
          >
            ...
            <div className="user-profile-hero">
              ...
              <div>
                ...
                <h2>{accountLabel}</h2>
                <p>{currentUser.email || 'Local account'}</p>
              </div>
            </div>
            ...
            <div className="profile-user-grid">
              ...
              <label className="profile-photo-picker">
                ...
                <input
                  ...
                  disabled={profileBusy || !firebaseConfigured}
                  onChange={(event) => {
                ...
                />
                <span className="outline-btn">{profileBusy ? 'UPDATING…' : 'SELECT PHOTO'}</span>
              </label>
            </div>

            <div className="profile-user-actions">
              <button className="primary-btn big" type="button" onClick={saveUserProfile} disabled={profileBusy}>
                {profileBusy ? 'PROCESSING…' : 'SAVE PROFILE'}
              </button>
              <button className="outline-btn big" type="button" onClick={resetUserPassword} disabled={profileBusy || !firebaseConfigured}>
                RESET PASSWORD
              </button>
            </div>
          </div>
