function requireTelegramConfig() {
  ...
  if (
    !token ||
    !chatId ||
    token === 'PUT_YOUR_NEW_BOTFATHER_TOKEN_HERE'
  ) {
    throw new Error(
...
export async function sendOrderReceiptForReview({
  imageFile,
  caption,
  orderId,
}) {
...
export async function sendReceiptImage(
  ...
  /*
   * Only attach the BUY / GET KEY button
   * when buyUrl is a valid HTTP/HTTPS URL.
   *
   * If the URL is invalid, the receipt will
   * still be sent without the button.
   */
  let validBuyUrl = '';
